package org.moja.spring.core.dao;

import org.moja.spring.core.entity.RequestSurvey;

import java.util.List;

public class RequestSurveyDAOImpl implements org.moja.spring.core.dao.impl.RequestSurveyDAO {
    @Override
    public void saveRequestSurvey(RequestSurvey requestSurvey) {

    }

    @Override
    public RequestSurvey editRequestSurvey(RequestSurvey requestSurvey) {
        return null;
    }

    @Override
    public RequestSurvey getRequestSurveyById(int requestSurveyID) {
        return null;
    }

    @Override
    public List<RequestSurvey> getAllRequestSurveys() {
        return null;
    }
}
